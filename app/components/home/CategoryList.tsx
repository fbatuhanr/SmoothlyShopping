        ...
        <div className="flex items-center justify-center w-full px-3 md:px-10 gap-2 md:gap-4 py-8 overflow-x-auto">
            {
                categories?.map((category:Category) => {
                    ...
                    return <Link href={targetUrl} key={category.id} className={`${isActive ? "bg-white border-gray-400 px-5 py-1 shadow-md drop-shadow shadow-slate-200" : "bg-slate-100 border-gray-200 px-4 py-2.5"} border text-slate-800  rounded-full min-w-[130px] flex items-center justify-center gap-x-2`}>
                        ...
                        <div className={`${isActive ? "text-lg font-extrabold" : "text-base font-semibold"}`}>{category.title}</div>
                    </Link>
                }
                )
            }
